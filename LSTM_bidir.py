...
功能:
  - 使用双向 LSTM(Bi-LSTM)对肌肉信号时间序列进行回归，预测物理参数 K 与 C。
...
输出:
  ...
  - 模型与归一化器保存到 model/:
    - model/bilstm_fusion_model.pth
    - model/bilstm_input_scaler.pkl
    - model/bilstm_target_scaler.pkl

运行:
  - 在项目根目录执行: python LSTM_bidir.py
"""
