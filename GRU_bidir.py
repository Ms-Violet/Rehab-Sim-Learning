...
# ================= 3. 双向 GRU 模型定义 =================
class RegressorBiGRU(nn.Module):
    def __init__(self, input_size, hidden_size=128, num_layers=2):
        super(RegressorBiGRU, self).__init__()
...
def train_and_test():
    ...
    # --- 初始化 BiGRU 模型 ---
    model = RegressorBiGRU(input_size=F).to(DEVICE)
    ...
    print("\n" + "="*60)
    print(f"Start Training (BiGRU Fusion) on {DEVICE}")
    ...
    print("="*60)
    print(f"Model Architecture:      Bi-GRU")
    ...
    with torch.no_grad():
        for x, y in test_loader:
            ...
            gts_log.append(y.cpu().numpy())
            
    # GRU 使用了 Log 变换，这里需要 exp 还原
    ...
    torch.save(model.state_dict(), "model/bigru_fusion_model.pth")
    print("Model saved.")
